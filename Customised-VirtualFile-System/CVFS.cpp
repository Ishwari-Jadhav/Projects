...
// File        :  CVFS.cpp
// Description :  Console-based virtual file system simulating basic UNIX-like
//                file operations with inodes and file descriptors management.
...
void InitialiseUAREA()
{
    ...
    cout<<"Marvellous CVFS : UAREA initialised successfully\n";
}
...
void StartAuxilaryDataInitialisation()
{
    ...
    cout<<"Marvellous CVFS : Auxilary data initalised successfully\n";
}
...
void DisplayHelp()
{
    ...
    printf("copy : Copy a file to a new file\n");
    printf("append : Append data to an existing file\n");
            ...
            )
{
    ...
    {
        printf("Description : Copy an existing file to a new file\n");
        printf("Usage : copy source_file_name dest_file_name\n");
    }
    else if(strcmp(name,"append") == 0)
    {
        printf("Description : Append data to an existing file\n");
        printf("Usage : append File_Descriptor\n");
    }
    else
    {
...
//  Description     :   It is used to diplsy the information about all 
...
//  Input           :   oldName  – Name of the existing file
//                      newName  – New name to be assigned to the file
//  Output          :   Returns EXECUTE_SUCCESS if renamed successfully,
...
int RenameFile(char *oldName, char *newName)
{
    ...
    while(temp != NULL)
    {
        if(strcmp(temp->FileName, oldName) == 0 && temp->FileType == REGULARFILE)
        {
...
//  Description     :   Copy content of one file to a new file.
//  Input           :   sourceName  – Name of the source file
//                      destName    – Name of the new destination file
//  Output          :   Returns EXECUTE_SUCCESS if copied successfully,
//                      otherwise appropriate error code
//  Author          :   Ishwari Pravin Jadhav
//  Date            :   10/08/2025
//
...
int CopyFile(char *sourceName, char *destName)
{
    ...
    if(IsFileExists(destName) == true)
    {
        return ERR_FILE_ALREADY_EXIST;
    }

    if(IsFileExists(sourceName) == false)
    {
        return ERR_FILE_NOT_EXIST;
    }
    ...
    while(temp != NULL)
    {
        if(strcmp(temp->FileName, sourceName) == 0 && temp->FileType == REGULARFILE)
        {
            int fd = CreateFile(destName, temp->Permission);
            if(fd < 0)
            {
                return fd;
            }

            strncpy(uareaobj.UFDT[fd]->ptrinode->Buffer, temp->Buffer, temp->ActualFileSize);
            uareaobj.UFDT[fd]->ptrinode->ActualFileSize = temp->ActualFileSize;
...
//  Description     :   Append data at the end of an existing file.
//  Input           :   fd    – File descriptor of the file
//                      data  – Address of the data buffer to append
//                      size  – Number of bytes to append
//  Output          :   Returns the number of bytes successfully appended
//                      or appropriate error code in case of failure
//  Author          :   Ishwari Pravin Jadhav
//  Date            :   10/08/2025
//
///////////////////////////////////////////////////////////////////////////
int AppendFile(int fd, char *data, int size)
{
    if(fd < 0 || fd >= MAXOPENEDFILES || uareaobj.UFDT[fd] == NULL)
    {
        return ERR_FILE_NOT_EXIST;
    }
    ...
    // Append data at the current write offset
    strncpy(uareaobj.UFDT[fd]->ptrinode->Buffer + uareaobj.UFDT[fd]->WriteOffset, data, size);

    // Update offsets and actual file size
...
int main()
{
    ...
    while(1)
    {
        ...
        {
            // Marvellous CVFS > append 3
            if(strcmp(Command[0], "append") == 0)
            {
                printf("Please enter the data you want to append: \n");
                fgets(InputBuffer, MAXFILESIZE, stdin);

                iRet = AppendFile(atoi(Command[1]), InputBuffer, strlen(InputBuffer)-1);

                if(iRet < 0)
                {
                    printf("Error appending data to file\n");
                }
                else
                {
                    printf("%d bytes appended successfully\n", iRet);
                }
                continue;
            }
        ...
        else if(iCount == 3)
        {
            ...
            else if(strcmp(Command[0], "read") == 0)
            {
                ...
                uareaobj.UFDT[atoi(Command[1])]->ReadOffset = 0;
            ...
            // Marvellous CVFS > rename old.txt new.txt
            ...
            else if(strcmp(Command[0], "rename") == 0)
            {
                ...
                {
                    printf("Error : New file name already exists\n");
                }
                else
                {
            ...
            // Marvellous CVFS > copy source.txt dest.txt
